...
where $\lambda_\alpha$ are the eigenvalues contained in the diagonal of $\bm{\Lambda}$ and $u_{i \alpha}$ the eigenvectors components found in $\mathbf{U}$. \\
We can also find the vector of word $j$, $\mathbf{w}_j = (w_{j\alpha})$, with 
